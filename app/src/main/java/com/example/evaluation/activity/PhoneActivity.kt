    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        /*binding.getCode.setOnClickListener {
            if(binding.telNumber.text.toString().length!=11){
                ...
                intent.putExtra("firstTel",binding.telNumber.text.toString())
...
/*
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if(resultCode== Activity.RESULT_OK){
            binding.telNumber.setText(data?.getStringExtra("firstTel"))
            CoroutineScope(Dispatchers.Main).launch {
                sendCode()
            }
        }
 ...
 /*   private suspend fun sendCode(){
    ...
    private suspend fun bindPhone(){
        try {
            val response: LoginResponse = EvaluationNetwork.bindPhone(token, binding.telNumber.text.toString())
                CoroutineScope(Dispatchers.Main).launch {
                    response.message.showToast()
                }
            if(response.code==200){
